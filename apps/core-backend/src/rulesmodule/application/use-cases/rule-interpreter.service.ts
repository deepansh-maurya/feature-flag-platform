...
export class RuleInterpreterService {
  ...
  async interpretAndValidate(input: InterpretRulesInput): Promise<Pick<RuleSetRecord, 'rules'>> {
    const { rawRules } = input;
    
    // Validate input
    if (!Array.isArray(rawRules) || rawRules.length === 0) {
      throw new BadRequestException('rawRules must be a non-empty array of strings');
    }

    // Get interpretation from OpenAI
    const interpreted = await this.openAIService.interpretRules(rawRules);
  ...
  private validateInterpretedRules(interpreted: any) {
    ...
    interpreted.rules.forEach((rule: any, idx: number) => {
      ...
      if (!rule.match || typeof rule.match !== 'object') {
        throw new BadRequestException(`Invalid match condition at index ${idx}`);
      }
